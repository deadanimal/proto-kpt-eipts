...
export class LoginComponent implements OnInit {
  ...
  // Image
  imgLogo = 'assets/img/logo/jata-logo.png'
  ...
  login() {
    ...
    this.successMessage()
    if (this.loginForm.value.username == 'pegawai') {
      this.authService.userRole = 1
      localStorage.setItem("userRole", "1");
      this.navigatePage('dashboard-admin')
    }
    else if (this.loginForm.value.username == 'pengguna') {
  ...
  navigatePage(path: String) {
    if (path == 'login') {
      ...
      return this.router.navigate(['/admin/penubuhan'])
    }
    else if (path == 'dashboard-user') {
      return this.router.navigate(['/user/penubuhan'])
    }
  }
